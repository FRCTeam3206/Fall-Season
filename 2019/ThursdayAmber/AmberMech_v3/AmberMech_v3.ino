...
void loop()
{
  ...
  int RightX    = pulseIn(ch2, HIGH);       // RC Right Y, wrist
  int RightY     = pulseIn(ch3, HIGH);       // Robot Y    shoulder
  int LeftY     = pulseIn(ch4, HIGH);       // Robot X    gripper
  ...
  if (
      ...
      )
    {
      ...
      if (debugOn)
      {
       ...
       int wrist_motor = (RightX - NEUTRAL) * K_WRIST;
       int gripper_servo = LeftY - NEUTRAL;
