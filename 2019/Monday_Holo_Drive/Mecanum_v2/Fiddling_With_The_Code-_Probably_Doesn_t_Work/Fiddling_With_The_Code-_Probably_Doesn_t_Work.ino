...
const bool debugOn = true;           // The overhead required by serial and delays would be bad for RC control. Easy to disable.
...
//    Serial.println(intakeRaw);
  //  delay(1000);                        // delay between writes to serial monitor
    ...
    long K_Drive_int;                                                      // Hopefully LeftKnob is in-bounds.
    K_Drive_int = 70;                                                      
    float K_Drive_float = K_Drive_int / 100.0; 
    float K_Strafe = min(1.2 * K_Drive_float, 1.0); 
    float drive   = K_Drive_float * (LeftY - NEUTRAL - 30);  
    float strafe  = K_Strafe * (NEUTRAL - LeftX - 370);
    ...
    float front_left  = drive + strafe  + rotate - 80;
    float front_right = drive - strafe  - rotate;
    float rear_left   = drive - strafe  + rotate;
    float rear_right  = drive + strafe  - rotate - 80;
