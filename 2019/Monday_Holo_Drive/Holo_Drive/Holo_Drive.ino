...
const int NEUTRAL    = 1500;          // Left Y couldnt be zeroed, so we changed the neutral.
void setup() {
...
//    Serial.print(LeftY);
    ...
    float drive   = K_Drive_float * (RightY - NEUTRAL);  
    float strafe  = K_Strafe * (NEUTRAL - RightX);

    float K_Rotate_float = 0.5;

    float rotate  = K_Rotate_float * (LeftX - NEUTRAL);
  ...
  if (debugOn) {
     ...
     */
        ...
        leftFrontMotor.writeMicroseconds(front_left + 1500);  

        if (abs(front_right) < PULSE_WIDTH_DEADBAND) front_right = 0;
        rightFrontMotor.writeMicroseconds(1500 - front_right);

        if (abs(rear_left) < PULSE_WIDTH_DEADBAND) rear_left = 0;   
        leftRearMotor.writeMicroseconds(rear_left + 1500);

        if (abs(rear_right) < PULSE_WIDTH_DEADBAND) rear_right = 0;
        rightRearMotor.writeMicroseconds(1500 - rear_right);
